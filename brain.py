        ...
        try:
            ...
            # ── SHELL ACCIÓN ──────────────────────────────────────────────
                ...
                try:
                    ...
                    # 1. Traducir lenguaje natural → función real de shell.py
                    #    (apagar, reiniciar, matar proceso, instalar, firewall...)
                    r = _despachar_shell_accion_por_keywords(texto_limpio, _shell)

                    # 2. Si no hubo match de keywords, degradar al texto crudo
                    #    (solo sirve si el usuario ya escribió un comando real)
                    if r is None:
                        r = _shell.ejecutar_controlado(
                            texto_limpio,
                            contexto=f"acción del sistema: {texto_limpio[:60]}"
                        )
