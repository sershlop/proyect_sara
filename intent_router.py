...
# ──────────────────────────────────────────────────────────────────────────
# CACHÉ DE VECTORES DE PROTOTIPOS — optimización de Capa B
# ──────────────────────────────────────────────────────────────────────────
# Antes: cada llamada a _capa_b_semantica() regeneraba el vector de los ~50
# prototipos en _PROTOTIPOS_CAPA_B desde cero (vía similitud_semantica, que
# llama generar_vector() dos veces por par). Eso es ~50 llamadas al modelo
# de embeddings POR CADA frase ambigua que llega — el origen real de los
# tiempos de espera largos reportados (hasta varios minutos).
# Ahora: los prototipos se vectorizan UNA sola vez la primera vez que se
# necesitan y se reutilizan en memoria. El texto del usuario también se
# vectoriza una sola vez por llamada (antes se vectorizaba una vez por cada
# uno de los 50 prototipos). Esto reduce el costo de Capa B de ~50
# encodes a ~1 encode + comparaciones coseno (baratas, sin tocar el modelo).
_VECTORES_PROTOTIPOS_CACHE: dict[str, list[tuple[str, list]]] = {}


def _obtener_vectores_prototipos() -> dict[str, list[tuple[str, list]]]:
    """
    Retorna {categoria: [(texto_prototipo, vector), ...]}, generando los
    vectores solo la primera vez que se llama. Llamadas siguientes devuelven
    el caché en memoria sin tocar el modelo de embeddings.
    """
    global _VECTORES_PROTOTIPOS_CACHE
    if _VECTORES_PROTOTIPOS_CACHE:
        return _VECTORES_PROTOTIPOS_CACHE

    try:
        import embeddings as _emb
    except Exception as e:
        _log("warning", "No se pudo cargar embeddings para cachear prototipos", str(e))
        return {}

    cache: dict[str, list[tuple[str, list]]] = {}
    for categoria, prototipos in _PROTOTIPOS_CAPA_B.items():
        vectores_cat = []
        for prototipo in prototipos:
            try:
                vec = _emb.generar_vector(prototipo)
                if vec is not None:
                    vectores_cat.append((prototipo, vec))
            except Exception:
                continue
        if vectores_cat:
            cache[categoria] = vectores_cat

    if cache:
        _VECTORES_PROTOTIPOS_CACHE = cache
        _log("debug", f"Prototipos de Capa B cacheados: {sum(len(v) for v in cache.values())} vectores en {len(cache)} categorías")
    return cache
...
def _capa_b_semantica(texto_norm: str) -> Optional[dict]:
    ...
    vectores_prototipos = _obtener_vectores_prototipos()
    if not vectores_prototipos:
        return None

    # Vector del texto del usuario: se genera UNA sola vez aquí, no una vez
    # por cada prototipo (antes eran ~50 generaciones por esta función).
    try:
        vector_texto = _emb.generar_vector(texto_norm)
    except Exception as e:
        _log("warning", "No se pudo vectorizar el texto en Capa B", str(e))
        return None

    if vector_texto is None:
        return None
    ...
    try:
        for categoria, prototipos_vec in vectores_prototipos.items():
            scores = []
            for _prototipo_texto, vector_prototipo in prototipos_vec:
                try:
                    similitud = _emb.similitud_coseno(vector_texto, vector_prototipo)
